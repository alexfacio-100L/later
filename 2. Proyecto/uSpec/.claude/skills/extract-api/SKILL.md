...
Read [api/agent-api-instruction.md](../../../references/api/agent-api-instruction.md). This is the **same** instruction file used by `create-api` — it defines the `ApiOverviewData` schema, property classification rules, naming conventions, and audit checklists.
...
The file schema is defined in [figma-plugin/docs/base-json-schema.md](../../../figma-plugin/docs/base-json-schema.md). Familiarize yourself with the top-level keys:
...
Downstream consumers: `../../../references/component-md/agent-component-md-instruction.md` Color body rendering (Strategy B) uses this mapping to relabel `stateColumns` headers from Figma names to `runtimeCondition`. Structure body rendering also uses it to relabel the `### State deltas` artifact columns when the deltas axis matches.
...
```json
{
  ...
  "data": {
    ...
    /* full ApiOverviewData object — matches the schema in ../../../references/api/agent-api-instruction.md */
  }
}
```
...
After `{componentSlug}-api.json` is written, project the `ApiDictionary` artifact as specified in the **`ApiDictionary` artifact** section of [api/agent-api-instruction.md](../../../references/api/agent-api-instruction.md). This is a pure re-projection — no new reasoning, no new Figma calls, no inference. Every field has a documented 1:1 source already captured in the api.json payload.
