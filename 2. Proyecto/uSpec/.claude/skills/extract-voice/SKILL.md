...
- [agent-screenreader-instruction.md](../../../references/screen-reader/agent-screenreader-instruction.md) — main instructions
- [voiceover.md](../../../references/screen-reader/voiceover.md) — iOS VoiceOver patterns
- [talkback.md](../../../references/screen-reader/talkback.md) — Android TalkBack patterns
- [aria.md](../../../references/screen-reader/aria.md) — Web ARIA patterns
...
**Evidence fast path (preferred).** When `evidencePath` is provided, read it first. If the file exists, parses as JSON, has `_meta.domain === "voice"`, and `_meta.baseSourceHash` matches `{cachePath}/{componentSlug}-prepare-manifest.json`, treat `data` as the Step 3 evidence set and proceed to Step 4 without rebuilding from raw `treeFlat[]`.

Populate the voice evidence structure by reading **only** from `_base.json` when the fast path is unavailable. Key mappings:
