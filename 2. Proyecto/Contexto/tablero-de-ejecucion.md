...
> **▶ Por dónde seguir:** la barata es **4.7** (`PhoneSolid` a 24×24, cinco minutos). De **1.17** solo queda `text/offer`, y **antes de tocarlo hay que ver dónde se usa** — si siempre va sobre `background/offer`, el token de encima es `text/onOffer` y puede que no necesite arreglo. *La cara es **4.2**: el defecto estructural de los iconos, con el eje ya unificado pero Solid y Outline en anatomías distintas.*
...
- [ ] **1.12 · Las sombras llegan a código como color plano — pero NO hay que reconstruirlas.** 🟢 **RE-DIAGNOSTICADA el 17 ago, y sale mucho más barata.**
  ...
  *Las tres tareas:* **(1)** añadir la capa `shadow` al pipeline · **(2)** borrar los 26 color styles falsos · **(3)** ~~tokenizar el color de sombra~~ ✅ **hecho el 17 ago**.
  ...
  🟡 *Anotado:* hay **dos colores de sombra** (`#0E1F35` en 25 estilos, `#051D3D` en los dos `Modern/Soft`) · **22 effect styles sin ligar** · y **13 opacidades para 27 sombras**, lo que indica que la escala de elevación no tiene sistema.
...
- [x] **1.20 · Los colores de estado no cambian con el mode, y por eso casi ninguno pasa AA en los dos.** 🔴 **Nace de una duda del Lead el 17 ago** y la auditoría la confirmó más grande de lo planteado.
  ...
  *Defecto original, ya cerrado:* **`text/secondary` en Dark es `#f82f56`**, el rojo de marca Web — un "texto secundario" rojo brillante que **falla sobre `background/secondary` (4.29)**; parece asignación equivocada. Y **`text/placeholder` en Dark es `#ffffff` puro, idéntico a `text/primary`** — el placeholder queda tan prominente como el texto real y **borra la señal de si el campo está lleno**. No es contraste, es usabilidad.
  ...
  🟡 **`background/positiveSubtle` queda anotado:** es el **único** `*Subtle` de fondo con valor saturado —sus tres hermanos son claros—. O su valor debería aclararse, o el token es un fondo de highlight y debería llamarse así. **Cambia el aspecto, así que es decisión del Lead.**
...
- [ ] **1.17 · La familia `link` está rota entera, y cada estado falla en un mode distinto.** Encontrado el 18 ago **convirtiendo la convención en un detector**: la sección 9 de `13-convencion-naming.md` (repo de Product Design) dice que un token que no cambia entre modes lleva sufijo `Static`, así que **todo token sin `Static` que valga lo mismo en Light y Dark o miente en su nombre o le falta el alias**. Barrido de los 98 tokens de color: **42 sin `Static` no cambian; 0 con `Static` sí cambian.**
  ...
  **Los tres estados de `link` fallan en sitios distintos**, así que ninguna revisión hecha en un solo mode los habría encontrado todos. `border/brand` es el peor en magnitud: azul marino casi negro sobre negro.
  ...
  🔴 **CORRECCIÓN del 19 ago: `text/offer` NO está bloqueado por ninguna decisión.** El texto anterior de esta tarea decía que su arreglo iba *"junto al renombre a `accent`"*, y **esa premisa se había invalidado el 14 de agosto**: `DECISIONS.md` registra que **`offer` NO entra en `accent`** —existe precisamente para no usar el rojo de error, un tag de *"últimos ladrillos"* con rojo de alarma comunica lo contrario de una oportunidad— y que **`icon/subtle` ya se renombró a `icon/accent`** en ese mismo lote.
  **Lo que sí queda por resolver es de contraste, no de nombre:** `text/offer` da **3.23** en Light. *Medido: `red/500` daría 5.10, pero es el mismo peldaño que `background/negative`, así que oscurecerlo lo acerca justo al rojo del que `offer` existe para diferenciarse.* **Antes de elegir hay que ver dónde se usa: si siempre va sobre `background/offer`, el token correcto encima es `text/onOffer` y `text/offer` podría no necesitar arreglo.**
...
- [ ] **2.1 · Los tres defectos de escala que se ven.**
  - `letterSpacing/t-*` no es monótono: `0.3 · 0.4 · 0.1 · 0.2` — dos inversiones.
  - Dos pares de escalones indistinguibles: `h-4xl`/`h-5xl` (ambos `-1.4px`) y `h-l`/`h-xl` (ambos `-0.6px`).
  - `lineHeight/t-n` tiene factor 1.200 — el más bajo del sistema, en el tamaño más pequeño, contra la regla que la escala misma sigue.
