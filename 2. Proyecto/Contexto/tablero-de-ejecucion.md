...
> **▶ Por dónde seguir:** **1.17** —los seis tokens de la familia `link` con contraste roto, cada uno en un mode distinto— es la candidata natural: es foundations, no depende de nadie, y arrastra el renombre de `text/offer` a `accent`. Las baratas son **4.7** (`PhoneSolid` a 24×24) y **1.15** (los tokens Highlight en `Tags`). *La cara es **4.2**: el defecto estructural de los iconos, ya con su eje unificado pero con Solid y Outline en anatomías distintas.*
...
- [ ] **1.15 · Los componentes ya revelaron tres malos usos y un token que faltaba.** Encontrado el 17 ago al revisar `Alerts` y `Tags` **a propuesta del Lead** — que fue el punto 2 del protocolo de cierre, no una distracción.
  ...
  *Hallazgo original, ya corregido:*
  ...
  ✅ **`Tags` `type=financial` tenía dos colores pegados a mano** —`#FFEDED` en `countDown`, `#D7F5E6` en `countUp`—: eran **los fondos de highlight financiero**. **Ya existen como tokens:** `background/negativeHighlight` y `background/positiveHighlight`, con sus primitivos `red/25` y `green/25`. Cierra la familia `Highlight`.
