...
> **Y la tarde del 19 ago, el cambio de librería de iconos:** **1.15** · **1.17** completa (los seis tokens de contraste, verificados en Supernova) · **4.8** (el swap de Heroicons a Phosphor, 591 instancias en once páginas) · **4.9** (el catálogo se conserva entero: Supernova cuenta sets, no variantes).
> **Y al final del día, foundations:** **1.8** (`positiveSubtle` fallaba 2.88 en siete tags; los tags pasan a `background/positive` y el arreglo destapó un 4.07 en Dark, corregido con `Static`) · **2.1** (el tracking no estaba mal diseñado sino **permutado por pares**; la escala queda monótona y sin duplicados).
> 🟢 **Y la corrección más útil del día sobre sombras: la capa `Shadow` de Supernova está VIVA y CORRECTA** — geometría entera y color tokenizado—, contra el diagnóstico del 14 ago que la daba por muerta. *Lo que falta de 1.12 es borrar 26 tokens Color a mano y esperar al puente técnico.*
...
>
> **▶ Por dónde seguir:** foundations queda limpio, y el bloque de residuos se aclaró el 19 ago. **La tarea grande es una sola y es tuya: borrar los 57 tokens residuales desde Supernova** —`Color → shadows/`, `graphs/` e `illustration/`—, que cierra **0.6** y **1.11** de un tirón. *Son estilos que ya no existen en Figma; el borrado manual es el único camino.* **1.12** solo espera el puente técnico para su capa `shadow`. **1.10** queda como decisión sobre `overlay/*`, no como deuda de aliasado. Y **4.10** cuando retomes documentación. 🔴 **Aparte de todo eso: el source `FigmaVariablesPlugin` está en `hasError`** —el trabajo de hoy sí llegó, pero conviene mirarlo. *`4.7` puede haber quedado sin objeto: `PhoneSolid` era un Heroicon.*
>
...
- [ ] **1.12 · Las sombras llegan a código como color plano — pero NO hay que reconstruirlas.** 🟢 **RE-DIAGNOSTICADA el 17 ago, y sale mucho más barata.**
  ...
  *Las tres tareas:* **(1)** añadir la capa `shadow` al pipeline · **(2)** borrar los 26 color styles falsos · **(3)** ~~tokenizar el color de sombra~~ ✅ **hecho el 17 ago**.
  ...
  🟢 **VERIFICADO el 19 ago comparando Figma contra Supernova lado a lado: la capa `Shadow` está VIVA, COMPLETA y SINCRONIZADA.** Los 26 effect styles de Figma están **ligados a variables `shadow/*`** y sus valores coinciden token a token con lo que Supernova entrega. Los 26 tokens de tipo `Shadow` traen **geometría entera y el color tokenizado `#0E1F35`** — `Shadows/Single/Small/sm-2` = `0 2px 4px 0 #0E1F35 / 12%`, y las compuestas con sus cuatro y cinco capas. **Y los dos `Modern/Soft` que se ligaron hoy llegaron con el color nuevo**, lo que prueba que **el import de estilos corre y las actualizaciones propagan.**
  ⚠️ *Eso corrige el diagnóstico del 14 ago, que daba los 26 `Shadow` por "desconectados, ninguno vivo". **Ya no lo están.***
  🔴 **Lo que sigue mal: los 26 `shadows/*` de tipo Color.** Siguen vivos, y **no están en "No collections": tienen colección asignada igual que los tokens buenos.** Lo que sí tienen es **`origin: null`** — son de los 57 que vienen del source tipo `Figma`, no del plugin de variables. 🔴 **Hipótesis refutada el 19 ago: re-importar el source NO los limpia.** Se borraron en Figma, se re-importó, y siguen ahí idénticos. **R-12 confirmado también para estilos: las eliminaciones no se propagan por ninguna vía automática.** *Hay que borrarlos a mano desde Supernova — el paso 3 del procedimiento de `10-renovare-later.md`.*
  ⚠️ **Y ahora estorban más que antes:** conviven **`Shadows/Single/Small/sm-1`** (tipo `Shadow`, correcto) y **`shadows/single/small/sm1`** (tipo `Color`, falso) — **mismo nombre salvo mayúsculas y un guion.**
...
- [x] **4.2 · Consolidar iconos:** fusionar Solid y Outline en un set con propiedad. ✅ **Sin objeto el 19 ago: la resolvió el cambio de librería.** Phosphor llega como **1512 component sets con propiedades `Format` y `Weight`** — la consolidación que esta tarea pedía construir *es* su estructura nativa. Verificado: **cero componentes sueltos y cero con nomenclatura `Solid`.** *Los cinco `Outline` que quedan son nombres propios de Phosphor —`DotOutline`, `ArrowsOutLineHorizontal`—, no la vieja convención.* ~~Son 309 Outline y 327 Solid, el 78% de los componentes root.~~
