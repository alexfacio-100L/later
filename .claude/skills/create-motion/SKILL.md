...
> **Out of scope of the Component Markdown (`.md`) consumer contract.** The other `create-*` skills require the component `.md` (produced by the `create-component-md` skill) and render from its `render-meta`. `create-motion` is the deliberate exception: its input is an After Effects timeline export (`../../../references/motion/export-timeline.jsx`), not a Figma component walk — there is no `_base.json` and no `extract-motion`. This skill does NOT require a `.md` and does NOT participate in the require-`.md`/fail-fast/whitelisted-reads refactor.
>
> ⚠️ **Local adequacy (Later, 31 Aug 2026). This clause used to end "and no Motion section in the `.md`", and that sentence is now false.** The `.md` template carries a `## Motion` section — but it is a different artifact on a different surface, not this skill's output. **This skill renders a detailed timeline spec INTO FIGMA from an After Effects export; the `.md` section states the motion INTENT an engineer implements** (what animates, how long, which easing, `prefers-reduced-motion`). *Neither one generates the other, and this skill still neither reads nor writes the `.md` — the scope declaration above is unchanged in substance.* **Do not delete the `## Motion` slot on the strength of the old wording.**
...
- the `create-motion` skill + pasted JSON (from clipboard, exported via `../../../references/motion/export-timeline.jsx`)
- the `create-motion` skill + a file reference (e.g., `@motion-ex.json` or a path to a `.json` file)
- the `create-motion` skill + a Figma link (destination page/file where the annotation should be placed)
...
Read [agent-motion-instruction.md](../../../references/motion/agent-motion-instruction.md)
...
> The motion spec template key is not configured. Run the `firstrun` skill with your Figma template library link first.
...
- The motion spec template key is stored in `uspecs.config.json` under `templateKeys.motionSpec` and is configured via the `firstrun` skill.
