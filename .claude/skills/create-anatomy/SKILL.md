...
// El fondo del padre evita que la imagen exportada salga transparente.
if (preview.fills && preview.fills.length) {
  try { wrapper.fills = JSON.parse(JSON.stringify(preview.fills)); wrapper.cornerRadius = preview.cornerRadius || 0; } catch {}
}
```

**Se agranda primero y se ajusta después**, en dos pasos: mover las piezas dentro de un frame que ya se encogió las recorta.
...
> The anatomy template key is not configured. Run the `firstrun` skill with your Figma template library link first.
...
4c. **Compose brief description**: Write a 1-sentence `briefDescription` (max ~15 words) that describes what this component IS and does — not what the spec shows. Incorporate user-provided context when available. See `../../../references/anatomy/agent-anatomy-instruction.md` for guidelines and examples. Save this string for Step 7.
